    ...
    awk '
        ...
        section == "mismatch" && /^  - / {
            ...
            if (length(checksum_fields) == 5 &&
                ...
                print target "\t" tolower(checksum_fields[2])
            }
            next
        }
        section == "missing" && /^  - / {
...
validate_and_print_model_repair_plan() {
    local destination=$1
    shift
    local repair_record repair_file expected_sha256
    printf '\nExactly these %d missing or corrupt model file(s) will be repaired:\n' "$#"
    for repair_record in "$@"; do
        IFS=$'\t' read -r repair_file expected_sha256 <<<"${repair_record}"
        validate_model_repair_target "${destination}" "${repair_file}"
        [[ "${expected_sha256}" == "MISSING" || "${expected_sha256}" =~ ^[[:xdigit:]]{64}$ ]] ||
            die "Repair plan contains an invalid checksum for ${repair_file}."
        printf '  - %s%s\n' "${repair_file}" \
            "$([[ "${expected_sha256}" == "MISSING" ]] && printf ' (missing)' || printf ' (SHA-256 %s)' "${expected_sha256}")"
    done
}
...
download_model_repair_files() {
    local repository=$1 repair_stage=$2
    shift 2
    local repair_record repair_file expected_sha256
    local -a repair_paths=()
    for repair_record in "$@"; do
        IFS=$'\t' read -r repair_file expected_sha256 <<<"${repair_record}"
        repair_paths+=("${repair_file}")
    done
    info "Downloading the exact repair list into an isolated staging directory"
    HF_HUB_DISABLE_TELEMETRY=1 "${HF_VENV_DIR}/bin/hf" download \
        "${repository}" "${repair_paths[@]}" --repo-type model --local-dir "${repair_stage}"
}

validate_staged_model_repair() {
    local repository=$1 destination=$2 repair_stage=$3
    shift 3
    local repair_record repair_file expected_sha256 actual_sha256
    local verification_output verification_tree

    for repair_record in "$@"; do
        IFS=$'\t' read -r repair_file expected_sha256 <<<"${repair_record}"
        [[ -f "${repair_stage}/${repair_file}" && ! -L "${repair_stage}/${repair_file}" ]] || {
            error "Hugging Face did not produce a regular repair file: ${repair_file}"
            return 1
        }
        actual_sha256="$(sha256sum -- "${repair_stage}/${repair_file}")"
        actual_sha256="${actual_sha256%% *}"
        if [[ "${expected_sha256}" != "MISSING" && "${actual_sha256}" != "${expected_sha256}" ]]; then
            error "Staged SHA-256 mismatch for ${repair_file}: expected ${expected_sha256}, got ${actual_sha256}"
            return 1
        fi
    done

    verification_tree="$(mktemp -d "$(dirname -- "${destination}")/.colibri-verify.XXXXXX")"
    cp -al -- "${destination}/." "${verification_tree}/" || {
        rm -rf --one-file-system -- "${verification_tree}"
        error "Could not create the hard-linked verification view."
        return 1
    }
    for repair_record in "$@"; do
        IFS=$'\t' read -r repair_file expected_sha256 <<<"${repair_record}"
        mkdir -p -- "${verification_tree}/$(dirname -- "${repair_file}")"
        rm -f -- "${verification_tree}/${repair_file}"
        ln -- "${repair_stage}/${repair_file}" "${verification_tree}/${repair_file}" || {
            rm -rf --one-file-system -- "${verification_tree}"
            error "Could not stage ${repair_file} in the verification view."
            return 1
        }
    done
    verification_output="$(mktemp)"
    if ! run_model_integrity "${repository}" "${verification_tree}" "${verification_output}"; then
        rm -f -- "${verification_output}"
        rm -rf --one-file-system -- "${verification_tree}"
        error "The complete staged model does not match the repository checksums."
        return 1
    fi
    rm -f -- "${verification_output}"
    rm -rf --one-file-system -- "${verification_tree}"
...
perform_model_repair() {
    ...
    if ! validate_staged_model_repair \
        "${repository}" "${destination}" "${repair_stage}" "${repair_files[@]}"; then
        rm -rf --one-file-system -- "${repair_stage}"
...
\t' read -r repair_file expected_sha256 <<<"${repair_record}"
        mkdir -p -- \
            "${repair_backup}/$(dirname -- "${repair_file}")" \
            "${destination}/$(dirname -- "${repair_file}")"
        if [[ -e "${destination}/${repair_file}" ]]; then
