...
readonly DEFAULT_UPSTREAM_REF="v1.2.0"
readonly DEFAULT_MODEL_REPO="mastouri/GLM-5.2-colibri-int4-g64-with-int8-mtp"
readonly DEFAULT_MODEL_REVISION="5276684ba30ac0026c07220d3f389171a84eb074"
...
Core commands:
  ...
  start                    Enable and start the service; Colibri loads the model
  stop                     Stop the service without removing anything
  restart                  Restart the service; Colibri loads the model
...
Model and storage:
  ...
  model verify [REPO] [DEST] [--revision REVISION]
  model repair [REPO] [DEST] [--revision REVISION] [--yes]
  model status [JOB]
  model attach [JOB]
  model resume [JOB]
  model cancel [JOB]
...
Defaults:
  ...
  Profile:      performance
  UI:           colibri-web
  Upstream:     Colibri v1.2.0 (updated only by `upgrade`)
  Model:        mastouri/GLM-5.2-colibri-int4-g64-with-int8-mtp
  Model pin:    5276684ba30ac0026c07220d3f389171a84eb074
EOF
}
...
load_config() {
    ...
    local has_model_revision="0"
    grep -q '^MODEL_REVISION=' "${CONFIG_FILE}" && has_model_revision="1"
...
write_config() {
    ...
    validate_model_repository "${MODEL_REPO}"
...
validate_model_revision() {
    local revision=$1
    [[ -n "${revision}" ]] || die "Model revision must not be empty."
    [[ "${revision}" =~ ^[A-Za-z0-9._/-]+$ ]] ||
        die "Invalid Hugging Face model revision: ${revision}"
    case "/${revision}/" in
        */../* | */./*) die "Unsafe Hugging Face model revision: ${revision}" ;;
    esac
}
...
has_only_non_model_repository_differences() {
    local verification_output=$1
    awk '
        function is_model_artifact(path) {
            return path ~ /\.safetensors$/ ||
        ...
        section == "mismatch" && /^  - / {
            ...
            sub(/: expected .*/, "", target)
            difference_count++
            if (is_model_artifact(target)) model_failure_count++
            next
        }
        section == "missing" && /^  - / {
            target = substr($0, 5)
        ...
        END {
            exit !(difference_count > 0 && model_failure_count == 0)
        }
    ' "${verification_output}"
}
...
run_model_integrity() {
    ...
    if ((status == 0)); then
        cat -- "${output_file}"
    elif has_only_non_model_repository_differences "${output_file}"; then
        cat -- "${output_file}"
        grep '^Warning:' "${output_file}" || true
        warn "Ignoring documentation, downloader metadata, and Colibri runtime sidecars; all AI-model runtime artifacts match revision ${revision}."
        status=0
    else
        cat -- "${output_file}" >&2
...
parse_model_repair_arguments() {
    ...
    local revision_variable=$3
    shift 3

    local -a positionals=()
    local requested_revision="${MODEL_REVISION}"
    local revision_was_requested="0"
    ...
    ((${#positionals[@]} <= 2)) ||
        die "Usage: ./colibri.sh model repair [MODEL_REPOSITORY] [MODEL_DIRECTORY] [--revision REVISION] [--yes]"

    printf -v "${repository_variable}" '%s' \
        "${positionals[0]:-${MODEL_REPO}}"
    printf -v "${destination_variable}" '%s' \
        "${positionals[1]:-${MODEL_DIR}}"
    if [[ "${revision_was_requested}" == "0" &&
        "${positionals[0]:-${MODEL_REPO}}" != "${MODEL_REPO}" ]]; then
        requested_revision="main"
    fi
    printf -v "${revision_variable}" '%s' "${requested_revision}"
}
...
build_model_repair_plan() {
    ...
    local revision=$2
    local destination=$3
    local repair_records_variable=$4
...
perform_model_repair() {
    ...
    if ! verify_completed_model_repair \
        "${repository}" \
        "${revision}" \
        "${destination}"; then
...
command_model_repair() {
    ...
    local destination
    local revision
    parse_model_repair_arguments repository destination revision "$@"
    validate_model_repository "${repository}"
    validate_model_revision "${revision}"
    destination="$(resolve_existing_model_directory "${destination}")"
    perform_model_repair "${repository}" "${revision}" "${destination}"
}

parse_model_verify_arguments() {
    local repository_variable=$1
    local destination_variable=$2
    local revision_variable=$3
    shift 3
    ...
    while (($#)); do
        case "$1" in
            ...
            --)
                ...
                ;;
            -*)
                die "Unknown model verify option: $1"
                ;;
            *)
                positionals+=("$1")
                shift
    ...
    ((${#positionals[@]} <= 2)) ||
        die "Usage: ./colibri.sh model verify [MODEL_REPOSITORY] [MODEL_DIRECTORY] [--revision REVISION]"
...
validate_source_checkout() {
    ...
    # Current stable upstream releases create this untracked build stamp while parsing
...
parse_common_options() {
    local model_repository_changed="0"
    local model_revision_changed="0"
    ...
    while (($#)); do
        case "$1" in
            ...
            --model-repo)
                ...
                MODEL_REPO=$2
                model_repository_changed="1"
                shift 2
                ;;
            --model-revision)
                [[ $# -ge 2 ]] || die "--model-revision requires a value."
                MODEL_REVISION=$2
                model_revision_changed="1"
                shift 2
                ;;
            --source-dir)
    ...
    if [[ "${model_repository_changed}" == "1" &&
        "${model_revision_changed}" == "0" ]]; then
        if [[ "${MODEL_REPO}" == "${DEFAULT_MODEL_REPO}" ]]; then
            MODEL_REVISION="${DEFAULT_MODEL_REVISION}"
        else
            MODEL_REVISION="main"
...
command_install() {
    ...
    if [[ -d "${MODEL_DIR}" ]]; then
        printf 'Existing model directory accepted without checksum or layout verification.\n'
        printf 'Colibri will inspect the configured model itself during startup.\n'
        if [[ "${NO_START}" == "0" ]]; then
            if [[ "${was_active}" == "1" ]]; then
                command_restart
            else
        ...
        sudo systemctl disable --now "${SERVICE_NAME}" >/dev/null 2>&1 || true
        if [[ "${NO_DOWNLOAD}" == "0" ]] &&
            confirm "The model directory is absent. Start the approximately 430 GB model download in GNU Screen?" yes; then
            command_model download "${MODEL_REPO}" "${MODEL_DIR}" --yes
        else
            warn "Installation is complete, but the service is stopped because the model directory does not exist."
...
command_model() {
    ...
    case "${action}" in
        download)
            ...
            local revision="${MODEL_REVISION}"
            if (($# >= 1)); then shift; fi
            if (($# >= 1)); then shift; fi
            if [[ "${repo}" != "${MODEL_REPO}" ]]; then
                revision="main"
            fi
        ...
        verify)
            ...
            local destination
            local revision
            parse_model_verify_arguments repo destination revision "$@"
            ...
            rm -f -- "${verification_output}"
            info "Model verification passed: all AI-model runtime artifacts match the pinned revision."
            ;;
        repair)
            command_model_repair "$@"
            ;;
